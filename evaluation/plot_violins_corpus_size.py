...
def main():
    ...
    for corpus_size in corpus_sizes:
        for target in targets:
            ...
            averages_path = args.base_path + '/out/' + args.corpus \
                            + '/sentence/' + corpus_size + '/results/' + args.model + '/' + args.setting + '/averages.' + target + '.txt'
            ...
            for i, record in enumerate(records[:10]):
                data_dicts.append({'CORPUS SIZE': corpus_size,
                                   'RANK': i+1,
    ...
    ax = sns.violinplot(x='RANK', y='STANDARD DEVIATION', hue='CORPUS SIZE',
                        data=data_df, palette=sns.cubehelix_palette(5, start=.5, rot=-.75), split=True)

    # Format chart.
    ax.set_title('Corpus Size Comparison')
    plt.tight_layout()

    # Save chart.
    plt.savefig(args.base_path + '/out/plots/violin_plot.corpus_size_comparison.'
                + args.corpus + '.sentence.' + args.model + '.' + args.setting + '.pdf')
    plt.clf()
